...
pub async fn run(
    ...
    if let Some(file_path) = from_file {
        ...
        println!("{}Importing from {}...", LINK, style(file_path.display()).cyan());
        let content = std::fs::read_to_string(&file_path)
            .with_context(|| format!("Failed to read file: {}", file_path.display()))?;
        
        relations_to_add = serde_json::from_str(&content)
            .with_context(|| "Failed to parse JSON. Expected array of {node1, node2, relation, type1?, type2?}")?;
        
        println!("{}Found {} relations to import", CHECK, style(relations_to_add.len()).green().bold());
    } else if interactive {
        // Interactive mode
        println!("{}Enter relations interactively (empty node1 to finish):", LINK);
        println!();
        ...
        loop {
            ...
            if node2_input.is_empty() {
                println!("  {} Node 2 cannot be empty, skipping...", style("⚠").yellow());
                continue;
            }
            ...
            if relation_input.is_empty() {
                println!("  {} Relation cannot be empty, skipping...", style("⚠").yellow());
                continue;
            }
            ...
            relations_to_add.push(RelationImport {
                ...
                type1: if type1_input.is_empty() { None } else { Some(type1_input) },
                type2: if type2_input.is_empty() { None } else { Some(type2_input) },
            });
